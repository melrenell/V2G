...
class CentralOptimization(object):
    ...
    def solve(self, project, net_load, real_number_of_vehicle, SOC_margin=0.05,
              SOC_offset=0.0, peak_shaving='peak_shaving', penalization=5, beta=None, plot=True):
        """Launch the optimization and the post_processing function. Results
    ...
    def initialize_net_load(self, net_load, real_number_of_vehicle, project):
        ...
        #This is taking the 1st 144 netload values 
        #(if time interval=10 because 1440/10=144)
        #and multiplying the values by scaling factor (25/14,000)
        #This is not sampling over the day
        #It is trying to resample over the time interval
        #But it can't because of the index
    ...
    def process(self, times, vehicles, d, pmax, pmin, emin, emax,
        ...
        with SolverFactory(solver) as opt:
            ...
            # ###### Set
            model.t = pyo.Set(initialize=times, doc='Time', ordered=True)
            last_t = model.t.last()
            model.v = pyo.Set(initialize=vehicles, doc='Vehicles')

            # ###### Parameters
            # Net load
            model.d = pyo.Param(model.t, initialize=d, doc='Net load')

            # Power
            model.p_max = pyo.Param(model.t, model.v, initialize=pmax, doc='P max')
            model.p_min = pyo.Param(model.t, model.v, initialize=pmin, doc='P min')
            
            # Energy
            model.e_min = pyo.Param(model.t, model.v, initialize=emin, doc='E min')
            model.e_max = pyo.Param(model.t, model.v, initialize=emax, doc='E max')

            model.e_final = pyo.Param(model.v, initialize=efinal, doc='final energy balance')
            # model.beta = Param(initialize=beta, doc='beta')

            # ###### Variable
            model.u = pyo.Var(model.t, model.v, doc='Power used')#domain=Integers, 

            # ###### Rules
            def maximum_power_rule(model, t, v):
                return model.u[t, v] <= model.p_max[t, v]
            model.power_max_rule = pyo.Constraint(model.t, model.v, rule=maximum_power_rule, doc='P max rule')
            
            def minimum_power_rule(model, t, v):
                return model.u[t, v] >= model.p_min[t, v]
            model.power_min_rule = pyo.Constraint(model.t, model.v, rule=minimum_power_rule, doc='P min rule')

            def minimum_energy_rule(model, t, v):
                return sum(model.u[i, v] for i in range(0, t + 1)) >= model.e_min[t, v]
            model.minimum_energy_rule = pyo.Constraint(model.t, model.v, rule=minimum_energy_rule, doc='E min rule')

            def maximum_energy_rule(model, t, v):
                return sum(model.u[i, v] for i in range(0, t + 1)) <= model.e_max[t, v]
            model.maximum_energy_rule = pyo.Constraint(model.t, model.v, rule=maximum_energy_rule, doc='E max rule')

            def final_energy_balance(model, v):
                return sum(model.u[i, v] for i in model.t) >= model.e_final[v]
            model.final_energy_rule = pyo.Constraint(model.v, rule=final_energy_balance, doc='E final rule')
            ...
            if peak_shaving == 'peak_shaving':
                ...
                model.objective = pyo.Objective(rule=objective_rule, sense=pyo.minimize, doc='Define objective function')
    ...
    def plot_result(self, model):
        ...
        result = pandas.concat([result, df], axis=1)
        ###################################result.to_csv("Result_from_netloadopt_summer.csv")

        M=1000000
        # Plot power constraints
        plt.subplot(221)
        plt.grid(b=None)
        plt.title('Power Constraints')
        plt.plot(result.index.values, result.pmax.values/M, label='pmax')
        plt.plot(result.index.values, result.power.values/M, label='power')
        plt.plot(result.index.values, result.pmin.values/M, label='pmin')
        plt.ylabel('Power [MW]')
        plt.legend(loc=4)
        ...
        plt.subplot(222)
        plt.grid(b=None)
        plt.title('Energy Constraints')
        plt.plot(result.index.values, result.emax.values/M, label='emax')
        plt.plot(result.index.values, result.powercum.values/M, label='powercum')
        plt.plot(result.index.values, result.emin.values/M, label='emin')
        plt.plot(result.index[-1], e_final/M, '*', markersize=15, label='efinal')
        ...
        plt.subplot(223)
        plt.grid(b=None)
        plt.title('Ramp Constraints')
        plt.plot(result.index.values, (result.ramppower.values + result.rampnet_load.values)/M, label='result ramp')
        plt.plot(result.index.values, (result.rampnet_load.values)/M, label='net_load ramp')
        plt.ylabel('Energy [MWh]')
        plt.legend(loc=1)
        ...
        plt.subplot(224)
        plt.grid(b=None)
        plt.title('Power Demand Results')
        plt.plot(result.index.values, result.net_load.values/M, label='net_load')
        plt.plot(result.index.values, (result.net_load.values + result.power.values)/M, label='net_load + vehicle')
        plt.legend(loc=2)
        plt.ylabel('Power [MW]')
        plt.show()
